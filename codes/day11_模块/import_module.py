...
# 
# 2、import modules (模块名字)  as  XX             #这里是导入整个模块的同时给它取一个别名，因为有些模块名字比较长，用一个缩写的别名代替在下次用到它时就比较方便
# 
# 3、from modules(模块名字)  import func(方法）或者变量      #从一个模块里导入方法或者变量 ，你要用到模块里的什么方法或者变量就从那个模块里导入那个方法或者变量，这样占用的内存就比较少
#
...
from codes.day06_dict.dict import *                 #导入dict模块下__all__列表里的变量
