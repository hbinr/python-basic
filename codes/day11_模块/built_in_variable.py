...
#可以看到，除了temp显示变量，还打印出了很多内置变量。重点学习下:__doc__，__file__，__name__，__package__ 这四个内置变量
...
print("builr_in_variable模块的__package__值为：" + (__package__ or '当前模块不属于任何包'))  #当前模块不属于任何包 注意()的使用
...
输出的结果有差异，原因是：当前模块运行后，被认作为师入口文件，在python中，入口文件和普通模块的内置变量是有区别的 
区别：
