"""
    ...
    通过导入Enum包来引入枚举类,定义的时候直接继承Enum即可,变量名推荐全大写
    个人经验：
...
# 类名.属性 会直接输出1吗？
print(Month.JANUARY)  # Month.JANUARY   如果是普通的类,应该输出的结果是1，但此处调用却输出了Month.JANUARY
                      # 事实上,code编码是可以自定义修改的,是1也好,2也罢.我们更关心是这个枚举值代表什么意义
