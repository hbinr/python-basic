...
这是因为IDEA自动去找要导入模块的路径，找到则会成功输出结果。而使用python命令去执行时，是在某路径下(指定好的)来执行命令，所以会报错
'''
# 二、绝对导入   是基于绝对路径而言的
'''
...
# 三、相对导入   是基于相对路径而言的，在python中 '.'表示当前目录路径，'..'表示上一级路径，'...'表示上上一级路径，以此类推
'''
在入口文件是无法使用相对导入来导入模块的，为什么呢？
因为相对导入，'.'是来获取__name__的值，而入口文件的__name__的值是'__main__'
因此：from .package1.package4.m4 temp4  等价于 from __main__.package1.package4.m4.m4   ,__main__下当然没有package4.m4了，
所以不能在入口文件使用相对导入的方式来导入包或模块

在/package1/package4/m4.py 来验证相对导入

思考：难道python的入口文件就仅支持绝对导入吗？答案是否定的，如果想要在入口文件使用相对导入，那么只需要'__name__'的
值不为'__main__'就可以了。。所以使用python -m 命令，将入口文件当作是模块来执行就OK了
实践:
第一步：cd 到codes目录
